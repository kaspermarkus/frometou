...
function Trash(){
	...
	this.display=function(){
		if(!kfm_vars.permissions.file.rm)
			return false;
		...
		if(kfm_theme=="blt")
			$j(el).attr('src','widgets/trash/trash_blt.png');
...
kdnd_addDropHandler('kfm_file','.widget_trash',
	function(e){
		if(!selectedFiles.length)
			kfm_addToSelection(e.sourceElement.id.replace(/.*_/,''));
		e.targetElement.action(selectedFiles,[]);
	});
kdnd_addDropHandler('kfm_dir_name','.widget_trash',
	function(e){
