...
function writeRichText(rte, html, width, height, buttons, readOnly) {
	if (isRichText) {
		...
		if (buttons == true) {
			...
			document.writeln('		<td><img class="rteVertSep" src="' + imagesPath + 'blackdot.gif" width="1" height="20" border="0" alt=""></td>');
			document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'hyperlink.gif" width="25" height="24" alt="Insert external Link" title="Insert External Link" onClick="insertLink(\'' + rte + '\')"></td>');
			...
			document.writeln('	</tr>');
			document.writeln('</table>');
			document.writeln('<table class="rteBack" cellpadding=2 cellspacing=0 id="Buttons3_' + rte + '" width="' + tablewidth + '">');
			document.writeln('	<tr>');
			document.writeln('		<td>');
			document.writeln('			<select id="addInternalImage_' + rte + '" onchange="addInternalImage(\'' + rte + '\', this.id);">');
			document.writeln('				<option value="">[Insert image]</option>');
			<?php
			$result = mysql_query("SELECT iid, small, ident FROM images ORDER BY ident ASC");
			while ($row = mysql_fetch_row($result)) {
				echo 'document.writeln("				<option value=\\"'.$row[1].'\\">'.$row[2].'</option>");'."\n";
			}
			?>
	 		document.writeln('			</select>');
			document.writeln('		</td>');
			...
			document.writeln('	</tr>');
			document.writeln('</table>');
			document.writeln('<table class="rteBack" cellpadding=2 cellspacing=0 id="Buttons4_' + rte + '" width="' + tablewidth + '">');
			document.writeln('	<tr>');
			document.writeln('		<td>');
			document.writeln('			<select id="addInternalLink' + rte + '" onchange="addInternalLink(\'' + rte + '\', this.id);">');
			document.writeln('				<option value="">[Insert internal link]</option>');
			<?php 
			$sql = "SELECT doc.did, dtype.ident, doc.ident FROM doc, dtype WHERE typeid=tid ORDER BY dtype.ident, doc.ident ASC";
			$num = null;
			if ($res = mysql_query($sql)) {
				while ($row = mysql_fetch_row($res)) {
					if ($num == $row[0]) {
						...
						echo 'document.writeln("                <OPTION VALUE=\\"'.$row[0].'\\">'.strtoupper($row[1]).': '.$row[2].'</OPTION>");'."\n";
						$num = $row[0];
					}
				}
			}
			...
			document.writeln('	</tr>');
			document.writeln('</table>');
			document.writeln('<table class="rteBack" cellpadding=2 cellspacing=0 id="Buttons5_' + rte + '" width="' + tablewidth + '">');
			document.writeln('	<tr>');
			document.writeln('		<td>');
			document.writeln('			<select id="addIndexListing' + rte + '" onchange="addIndexListing(\'' + rte + '\', this.id);">');
			document.writeln('				<option value="">[Insert index-listing]</option>');
			document.writeln('				<option value="regularType">List ordered by types, with flags</option>');
			document.writeln('				<option value="simple">Simple list with flags</option>');
			document.writeln('				<option value="withDescription">With description and image</option>');
	 		document.writeln('			</select>');
			document.writeln('		</td>');
			document.writeln('		<td width="100%">');
			document.writeln('		</td>');
...
function toggleHTMLSrc(rte) {
	...
	if (document.getElementById("chkSrc" + rte).checked) {
		...
		showHideElement("Buttons3_" + rte, "hide");
		showHideElement("Buttons4_" + rte, "hide");
		showHideElement("Buttons5_" + rte, "hide");
		if (document.all) {
		...
		showHideElement("Buttons3_" + rte, "show");
		showHideElement("Buttons4_" + rte, "show");
		showHideElement("Buttons5_" + rte, "show");
		if (document.all) {
			//fix for IE
...
function insertLink(rte) {
	...
	var szURL = prompt("Enter an URL to link to:", "");
	try {
...
function addInternalLink(rte, selectname) {
	var idx = document.getElementById(selectname).selectedIndex;
	// First one is always a label
	if (idx != 0) {
		var selected = document.getElementById(selectname).options[idx].value;
		rteCommand(rte, 'CreateLink', selected);
...
function addInternalImage(rte, selectname) {
...
function addIndexListing(rte, selectname) {
	//function to handle inserts of index listings
	var idx = document.getElementById(selectname).selectedIndex;
	// First one is always a label
	if (idx != 0) {
		var selected = document.getElementById(selectname).options[idx].value;
		selected = "##indexList::"+selected+"##";
		//rteCommand(rte, 'Inserthtml', selected);
		insertTxt(rte, selected);
		//insertAtCursor(rte, "KASPER");
		document.getElementById(selectname).selectedIndex = 0;
	}
}

function insertTxt(rte, txt) {
	//function to perform command
	var oRTE;
 	if (document.all) {
 		oRTE = frames[rte];
