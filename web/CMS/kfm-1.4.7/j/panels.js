...
function kfm_addPanel(wrapper,panel){
    ...
    if(panel=='kfm_directories_panel'){
        ...
        el=kfm_createPanel(kfm.lang.Directories,'kfm_directories_panel',t,{
            'state':1,
            'abilities':-1,
            'order':1
        });
        ...
        el=kfm_createPanel(kfm.lang.DirectoryProperties,'kfm_directory_properties_panel',t,{
            state:0,
            abilities:1
        });
    }
    ...
    else if(panel=='kfm_local_file_selector')el=kfm_localFileSelector();
    else if(panel=='kfm_search_panel')el=kfm_createSearchPanel();
    else if(panel=='kfm_widgets_panel')el=kfm_createWidgetsPanel();else return;
    if(!wrapper.panels)wrapper.panels=[];
    wrapper.panels[wrapper.panels.length]=panel;
    wrapper.appendChild(el);
}

function kfm_localFileSelector(){
	var spacer1 = document.createElement("p");
	var spacer2 = document.createElement("p");
	var spacer3 = document.createElement("p");
	//create select box:
	var sel2 = document.createElement("select");
	sel2.id = 'localFileSelector';
	//kfm_local_file_list -- see kfm/index.php
	jQuery.each(kfm_local_file_list, function(index, value) { 
		jQuery(sel2).append(jQuery("<option></option>").attr("value",value.key).text(value.value));

	});
	var selectLocalButton = document.createElement("input");
	selectLocalButton.type='button';
	selectLocalButton.value='Add Link';
	jQuery(selectLocalButton).bind('click', function() {
		if (window.opener.CKEDITOR) {
			var funcnum = document.location.toString().replace(/.*CKEditorFuncNum=([0-9]*)[^0-9].*/, '$1');
			var selectedPage = "/dk/page"+jQuery("#localFileSelector").val();
			window.opener.CKEDITOR.tools.callFunction(funcnum, selectedPage);
			setTimeout('window.close()', 1);
		};
	});
    var contents=[spacer1,sel2,spacer2,selectLocalButton,spacer3];
    return kfm_createPanel("Link to local file",'kfm_local_file_selector',contents,{
        maxedState:3,
        state:3,
        order:2
    });
...
function kfm_createFileUploadPanel(contentsonly){
    var sel=newSelectbox('uploadType',[kfm.lang.Upload,kfm.lang.CopyFromURL],0,0,function(){
        ...
        var unzip1=document.getElementById('kfm_unzip1'),
	    unzip2=document.getElementById('kfm_unzip2'),
	    file=document.getElementById('kfm_file'),
	    url=document.getElementById('kfm_url');
    ...
    wrapper.appendChild((nm||!kfm_vars.use_multiple_file_upload)?kfm_fileUploadForm_native():kfm_fileUploadForm_flash());
    ...
    return contentsonly?contents:kfm_createPanel(kfm.lang.FileUpload,'kfm_file_upload_panel',contents,{
        maxedState:3,
        state:3,
        order:2
    });
...
function kfm_createFileDetailsPanel(){
    return kfm_createPanel(kfm.lang["file details"],'kfm_file_details_panel',0,{
        abilities:1,
        order:4
    });
}
function kfm_createPanel(title,id,subels,vars){
    ...
    var el=$extend(panelEl,{
        ...
        addMaxButton:function(){
            this.addButton('maximisePanel','','M',kfm.lang.Maximise)
            },
        addMinButton:function(){
            this.addButton('minimisePanel','','_',kfm.lang.Minimise)
            },
        addMoveDownButton:function(){
            if(this.id!=this.parentNode.panels[this.parentNode.panels.length-1])this.addButton('movePanel',',1','d',kfm.lang.MoveDown)
                },
        addMoveUpButton:function(){
            if(this.id!=this.parentNode.panels[0])this.addButton('movePanel',',-1','u',kfm.lang.MoveUp)
                },
        addRestoreButton:function(){
            this.addButton('restorePanel','','r',kfm.lang.Restore)
            },
...
function kfm_createSearchPanel(contentsonly){
    ...
    return kfm_createPanel(kfm.lang.Search,'kfm_search_panel',t,{
        maxedState:3,
        state:3,
        order:3
    });
...
function kfm_createWidgetsPanel(){
    ...
    el=kfm_createPanel('Widgets','kfm_widgets_panel',widgets,{
        'state':3
    });
    return el;
}
function kfm_hasPanel(wrapper,panel){
    for(var i=0;i<wrapper.panels.length;++i)if(wrapper.panels[i]==panel)return true;return false;
}
...
function kfm_movePanel(wrapper,panel,offset){
    var i=0,j,k;
    for(;i<wrapper.panels.length;++i)if(wrapper.panels[i]==panel.id)j=i;if(offset<0)--j;
    k=wrapper.panels[j];
...
function kfm_refreshPanels(wrapper){
    ...
    for(i=0;i<ps.length;++i){
        ...
        if(el.visible){
            ...
            switch(el.state){
                case 0:
                    minimised[minimised.length]=ps[i];
                    break;
                case 1:
                    maximised[maximised.length]=ps[i];
                    break;
                case 2:
                    fixed_height[fixed_height.length]=ps[i];
                    break;
                case 3:
                    fixed_height_maxed[fixed_height_maxed.length]=ps[i];
                    break;
            }
        }
