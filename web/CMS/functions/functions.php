...
function selectIndex($header, $name, $size) {
	$sql = "SELECT did, ident FROM doc WHERE typeid = 0";
	$sql .= " ORDER BY ident ASC";
	return headerSelectBox($header, $sql, $name, $size, null);
}

function selectParent($header, $name, $size, $id) {
	$sql = "SELECT hierarchy.parent, ident FROM hierarchy, doc WHERE ";
	$sql .= " doc.did = hierarchy.parent AND hierarchy.did = $id ";
	$sql .= " ORDER BY ident ASC";
	return headerSelectBox($header, $sql, $name, $size, null);
}

function selectChild($header, $name, $size, $id) {
	$sql = "SELECT doc.did, doc.ident FROM hierarchy, doc WHERE ";
	$sql .= "doc.did = hierarchy.did AND hierarchy.parent = $id ";
	$sql .= " ORDER BY ident ASC";
	return headerSelectBox($header, $sql, $name, $size, null);
}
