...
if (isset($_POST['submit'])) {
	...
	if ($errormsg != "") {
		...
		if ($_POST['subscribe_check'] == "true") {
			...
			$subject = "Successfully subscribed to Fundamentalism mailing list"; //Mailens titel
			$message = "Welcome ".$_POST['fullname']."\r\n\r\n";
			$message .= "\tYou have successfully subscribed to the maillinglist of fundamentalism.dk.\r\n\r\n";
			$message .= "\tIf an error occured, or you for some reason wish to be removed from the maillinglist,";
			$message .= "please write so to aidoh@aidoh.dk.";
			...
			mysql_query($query);
			//echo $query;
			$successmsg = "<H2>Welcome to the Fundamentalism network</H2>";
			$successmsg .= "You have successfully been subscribed to the mailling list of Fundamentalism";
			$successmsg .= "<HR><BR>";
 		}
		/* ------------- SUPPORT LIST ------------------------- */
		if ($_POST['support_check'] == "true") {
			...
			$to = $_POST['fullname']." <".$_POST['email'].">";
			$from = "From: The Jens Galschiøt - Fundamentalism <aidoh@aidoh.dk>"; 
			$subject = "Confirm your support of Fundamentalism"; //Mailens titel
...
<?php echo $vars['newsletter checkbox']; ?>	<INPUT <?php if (!isset($_POST['submit']) || $_POST['subscribe_check'] == "true") echo "CHECKED='checked'"; ?> name="subscribe_check" value="true" type="checkbox"><?php echo $vars['newsletter checkbox']; ?><br>
